...
import data.db.tables.order.OrderItemsTable
import data.db.tables.order.OrdersTable
...
class OrderRepository(private val database: Database) {
    ...
    suspend fun placeOrder(order: OrderEntity): String = dbQuery {
    ...
    suspend fun getOrdersByUser(userId: String): List<OrderEntity> = dbQuery {
    ...
    suspend fun getOrderById(orderId: String): OrderEntity? = dbQuery {
    ...
    suspend fun updateOrderStatus(orderId: String, newStatus: String): Unit = dbQuery {
